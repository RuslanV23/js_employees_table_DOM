...
class MyTable {
   ...
   */ setActionBodyRow(newValue) {
    ...
    setActionHead(newValue) {
        ...
        if (oldValue === newValue) sortOrder = newValue.dataset.sortOrder === "ASC" ? "DESC" : "ASC";
    ...
    startEvents() {
       ...
       */ const selectedCell = e.target.closest("td");
            if (!e) return;
            if (!selectedCell || this.editingCell) return;
            const input = document.createElement("input");
            const originalInput = selectedCell.textContent;
            const styles = getComputedStyle(selectedCell);
            const width = selectedCell.clientWidth - parseFloat(styles.paddingLeft) - parseFloat(styles.paddingRight);
            ...
            input.addEventListener("blur", (e2)=>{
                ...
                input.remove();
                this.editingCell = null;
                if (inputValue === "") selectedCell.textContent = originalInput;
                else selectedCell.textContent = inputValue;
            });
    ...
    sort(cellHead, sortOrder) {
        if (cellHead === null) return;
        const indexColumn = cellHead.cellIndex;
        const isNumberColumn = cellHead.textContent.trim() === "Age" || cellHead.textContent.trim() === "Salary";
        const bodyRows = [
            ...this.tBody.rows
        ];
        bodyRows.sort((rowA, rowB)=>{
            ...
            if (sortOrder === "ASC") {
                if (isNumberColumn) return this.strToNumber(cellA) - this.strToNumber(cellB);
                return cellA.localeCompare(cellB);
            } else {
                if (isNumberColumn) return this.strToNumber(cellB) - this.strToNumber(cellA);
...
function createForm() {
    ...
    form.addEventListener("submit", (e)=>{
        ...
        if (!formData.name || !formData.position || !formData.office || !formData.age || !formData.salary) return showNotification("Please fill all fields", true);
        if (formData.name.length < 4) return showNotification("Name must be at least 4", true);
        if (formData.age < 18 || formData.age > 90) return showNotification("Age must be between 18 and 90", true);
